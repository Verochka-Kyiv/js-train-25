...
async function logRandomNumberAfterSeconds(seconds) {
  ...
  try {
    ...
    console.error("Помилка при отриманні випадкового числа:", error);
  }
}
...
async function displayData() {
  ...
  const { value: dbValue } = await dataGenerator.next();
  const { value: apiValue } = await dataGenerator.next();
  const { value: cacheValue } = await dataGenerator.next();
